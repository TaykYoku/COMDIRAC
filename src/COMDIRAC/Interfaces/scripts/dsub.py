...
Script.registerArgument("executable: command to be run inside the job. If a relative path, local file will be included in InputSandbox. If no executable is given and JDL (provided or default) doesn't contain one, standard input will be read for executable contents", mandatory=False)
Script.registerArgument(["arguments: arguments to pass to executable if some arguments are to begin with a dash '-', prepend '--' before them"], mandatory=False)
